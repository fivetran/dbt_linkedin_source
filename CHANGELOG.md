...
## Breaking Changes
- The `click_uri_type` field has been added to the `stg_linkedin_ads__creative_history` model. This field allows users to differentiate which click_uri type (`text_ad` or `spotlight`) is being used to populate the results of the `click_uri` field.
